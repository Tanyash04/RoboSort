...
def setup_environment():
    ...
    #p.changeVisualShape(plane, -1, rgbaColor=[0.1, 0.1, 0.25, 1])  # Increase blue intensity
    #p.changeVisualShape(cup, -1, rgbaColor=[1, 0.6, 0.8, 1])  # Force pink appearance
   # p.changeVisualShape(lid, -1, rgbaColor=[1, 1, 1, 1])  # Ensure white lid remains white
    #p.changeVisualShape(plane, -1, rgbaColor=[0.05, 0.05, 0.1, 1])  # Darker blue
    ...
    lid_visual = p.createVisualShape(
        shapeType=p.GEOM_MESH,
        fileName=r"./3dmodels/Lid_15n.obj",
        meshScale=[1.5, 1.5, 1.5],
...
def move_to_target(robot, target_pos, ee_index, steps=200):
    ...
    for _ in range(steps):
        ...
        p.stepSimulation()
        time.sleep(0.001)  # Reduce sleep time to speed up simulation
...
def run_simulation(env):
    ...
    grasp_x, grasp_y, grasp_z = 0.253,	0.284,	0.27
    ...
    if verify_grasp(robot, lid):
        ...
        p.changeDynamics(lid, -1, mass=10, lateralFriction=500, spinningFriction=250, rollingFriction=150)
...
# This block is no longer needed when running with Flask
